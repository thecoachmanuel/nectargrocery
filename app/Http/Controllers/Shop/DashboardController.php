...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $pending    = (clone $orderObject)->where('order_status', 'Pending')->count();
        $confirm    = (clone $orderObject)->where('order_status', 'Confirm')->count();
        $processing = (clone $orderObject)->where('order_status', 'Processing')->count();
        $pickup     = (clone $orderObject)->where('order_status', 'Pickup')->count();
        $onTheWay   = (clone $orderObject)->where('order_status', 'On The Way')->count();
        $delivered  = (clone $orderObject)->where('order_status', 'Delivered')->count();
        $cancelled  = (clone $orderObject)->where('order_status', 'Cancelled')->count();

        return view('shop.dashboard', compact(
            'totalOrder', 'totalProduct', 'orderStatuses',
            'topSellingProducts', 'topReviewProducts', 'latestOrders',
            'topFavorites', 'totalBrand', 'totalWithdraw', 'totalPosSales',
            'totalDeliveryCollected', 'pendingWithdraw', 'alreadyWithdraw',
            'deniedWithdraw', 'flashSale',
    ...
    {
        $shop = generaleSetting('shop');
        $notifications = \App\Models\Notification::where('shop_id', $shop?->id)
            ->orderBy('is_read', 'asc')->latest('id')->take(10)->get();
        $total = \App\Models\Notification::where('shop_id', $shop?->id)->where('is_read', false)->count();
