...
class OrderController extends Controller
{
    ...
    public function store(OrderRequest $request)
    {
        ...
        $paymentInput = strtoupper((string) $request->payment_method);
        $paymentMethod = PaymentMethod::CASH;
        foreach (PaymentMethod::cases() as $case) {
            if (strtoupper($case->name) === $paymentInput || strtoupper($case->value) === $paymentInput) {
                $paymentMethod = $case;
                break;
            }
        }
        if (!in_array($paymentInput, ['CASH', 'CASH_ON_DELIVERY']) && $paymentMethod === PaymentMethod::CASH) {
        ...
        if ($paymentMethod !== PaymentMethod::CASH) {
            $paymentUrl = route('order.payment', ['payment' => $payment->id, 'gateway' => $request->payment_method ?? 'card']);
        }
