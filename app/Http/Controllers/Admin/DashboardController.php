...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // Per-status counts for dashboard order status boxes
        $pending = Order::when($shop, fn($q) => $q->where('shop_id', $shop?->id))->where('order_status', 'Pending')->count();
        $confirm = Order::when($shop, fn($q) => $q->where('shop_id', $shop?->id))->where('order_status', 'Confirm')->count();
        $processing = Order::when($shop, fn($q) => $q->where('shop_id', $shop?->id))->where('order_status', 'Processing')->count();
        $pickup = Order::when($shop, fn($q) => $q->where('shop_id', $shop?->id))->where('order_status', 'Pickup')->count();
        $onTheWay = Order::when($shop, fn($q) => $q->where('shop_id', $shop?->id))->where('order_status', 'On The Way')->count();
        $delivered = Order::when($shop, fn($q) => $q->where('shop_id', $shop?->id))->where('order_status', 'Delivered')->count();
        $cancelled = Order::when($shop, fn($q) => $q->where('shop_id', $shop?->id))->where('order_status', 'Cancelled')->count();

        return view('admin.dashboard', compact(
            'totalShop', 'totalOrder', 'totalCustomer', 'totalProduct', 'orderStatuses',
            'topCustomers', 'topSellingProducts', 'topReviewProducts', 'topShops',
            'latestOrders', 'topFavorites', 'pendingWithdraw', 'alreadyWithdraw',
            'deniedWithdraw', 'totalCommission', 'totalCategories', 'flashSale',
            'totalRider', 'businessModel', 'generaleSetting',
    ...
    {
        $notifications = \App\Repositories\NotificationRepository::query()
            ->whereNull('shop_id')->whereNull('user_id')
            ->orderBy('is_read', 'asc')->latest('id')->take(10)->get();

        $total = \App\Repositories\NotificationRepository::query()
            ->whereNull('shop_id')->whereNull('user_id')->whereIsRead(false)->count();

        return $this->json('notifications', [
            'total' => $total >= 10 ? '9+' : $total,
            'notifications' => \App\Http\Resources\WebNotificationResource::collection($notifications),
        ]);
    }
