<template>
    <div class="main-container pt-8 pb-12">
        <div
            v-if="!isLoading"
            class="text-slate-800 text-lg lg:text-3xl font-bold"
        >
            {{ $t("Most Popular") }}
        </div>
        <!-- loading -->
        <SkeletonLoader
            v-else
            class="w-48 sm:w-60 md:w-72 lg:w-96 h-12 rounded-lg"
        />

        <!-- Products -->
        <div
            class="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-6 items-start mt-6"
        >
            <div
                v-if="!isLoading"
