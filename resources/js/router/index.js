...
const routes = [
    ...
    {
        // Navbar links still point here (/categories/:id[?subcategory=x]).
        // Stash the clicked ids in the store and send the user to a clean
        // /products URL (no query params); Products.vue consumes it once to
        // preselect the option. Reload shows the plain full list.
        path: "/categories/:slug",
        name: "category-product",
        beforeEnter: (to) => {
            const master = useMaster();
            master.pendingCategory = {
                category: Number(to.params.slug),
                subcategory: to.query.subcategory
                    ? Number(to.query.subcategory)
                    : null,
            };
